...
> 2026-08-10 现行替代：主题管理 AI 生成领域、主题、全部笔记归属及归属确定后的每主题唯一整合；整合同时持有并展示该次修订自动关联的笔记来源。主题管理卡三与主题洞察“主题整合”读取同一当前已应用修订。主题洞察 AI 只生成综述、竞争假设、判断演变和决策与行动；已确认正式记录独立保留。旧确定性分类器、个人目录、关键词/别名评分、65 分门槛及全部本地语义生成已删除。下文仍出现的旧分类规则只作历史依据，不得恢复为运行能力。
>
> 没有已应用AI全库分类修订时，主题管理不得显示旧领域/主题、旧来源归属或`已归入当前主题`一类兜底；卡二与卡三保持空白等待态。全库生成先产生可审核草稿，只有应用后才成为正式分类。生成过程与完成/失败结果必须使用持久弹窗反馈。
...
- 新来源完成保真导入后进入全部笔记，并等待下一次 AI 全库分类修订；低置信或冲突项由 AI 标为待核对，不恢复本地候选评分或独立收录箱入口。
...
- 接入通道允许`openrouter / deepseek_direct / qwen_direct`；OpenAI 与 Claude 不建立直连通道。
- OpenRouter 与 DeepSeek 目录从供应商接口动态读取；千问只显示已核对的`qwen3.7-flash / qwen3.7-plus / qwen3.8-max-preview`，不调用不保证支持的`GET /models`。首次真实调用才验证 API Key、地区和模型权限。
- 千问全库分类的唯一阶段路由为：逐篇档案、批量归属→`qwen3.7-flash`；跨文档结构、主题整合→`qwen3.7-plus`。主题洞察默认用 Plus；高难判断必须由用户明确选择`qwen3.8-max-preview`或`deepseek-v4-pro`，禁止隐式升级、降级、跨通道回退或消耗其他通道额度。
- API Key 只保存在 Windows 凭据库；数据库只保存通道、模型目录、选择、任务用量、费用和派生洞察。
- 普通设置读取只返回凭据是否存在；已配置输入框持续显示掩码。只有用户点击显示图标时才允许按当前通道临时读取明文，切换通道后恢复掩码。
- 普通总结、关键洞察和关系建议可直接生成派生结果；主题合并、删除、改名和覆盖人工判断/证据不得自动执行。
- 每次任务记录供应商、模型、成功/失败、输入/输出/推理/缓存 Token 以及实际或估算美元费用；多模型的全库任务还按`profiles / structure / assignments / integrations`保存阶段模型与该批用量。无法可靠计价时明确记为不可用，不伪造金额。
