...
- 暗色下所有表面继续服从同一深色材质总线；普通按钮、搜索历史、筛选、菜单、弹窗、输入、Markdown表格/引用/代码块等控制或高密度阅读面保持实体。唯一透明例外见 2026-08-14 合同：暗色场景玻璃的普通最前景业务卡使用同皮肤深色面 50% 透明度；危险操作仍允许深红语义面，任何暗色面都不得回退为白底。
- 暗色模式不改变页面结构、功能位置、几何、滚动、点击范围或交互职责。场景皮肤可保留外层背景与玻璃，并按 2026-08-14 合同让普通最前景业务卡透景；实体皮肤与所有控制层仍使用不透明材质总线。
...
- 2026-08-14 南烛枫明确替代暗色场景前景卡的旧不透明约束：只有`暗色 + 场景玻璃（花房/奔马）`组合下，所有普通最前景业务卡统一消费`--dark-frontmost-*`并使用 50% 透明度，让背景在暗色卡面下可见；三套暗色实体皮肤、五套浅色皮肤仍保持不透明。搜索历史、筛选、菜单、弹窗、输入和其他可操作控制层不跟随该透明度，继续使用实体底；PDF等原生文档画布保持内容原貌。此条只替代旧合同中“暗色场景内部业务卡不透明”的 D 材质部分，不改变页面几何、功能、滚动和交互。
...
| NF-FOREGROUND-OPAQUE-22 | `screenshots/final-core-workspace/foreground-cards-translucent-reference-20260810.png` | `3427AC4252F1A0DDC79280ADE3726F3398A1D4C6FCA30700BB7D1879B170DA5C` | D/F历史问题基准：2026-08-10真实WebView2截图确认当时浅色前景卡透明后显灰，因而亮色与实体皮肤前景卡锁定为不透明。2026-08-14新增的暗色场景玻璃50%专属例外只替代暗色花房/奔马，不把本失败图扩展为新效果，也不改变弹层、控制层、外层承托、示例内容或布局。 |
