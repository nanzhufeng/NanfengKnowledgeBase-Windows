...
| Topic / Subtopic | 长期主题、唯一主路径和横向关系 | `knowledge/repository.rs` | `createTopic/listTopics/getTopicDetail` | 主题浏览器、整理工作台 | 用 Record 标题自动生成主题 | 正式仓库与 UI 已接入；合并旧名称和旧路径写入 redirect 别名且撤销可逆 |
...
| Classification Suggestion | 来源到主题的候选、分数、理由和状态 | 评分：`deterministicClassifier.ts`；持久化输入/确认：`knowledge/repository.rs` | `prepare/save/list/confirm/undoClassification` | 收录箱 | 分类器直接写库；页面复制评分规则 | 统一输入、个人目录确认、规则/别名/实体 CRUD 和纠正反馈均已接入 |
...
- 正式 `D:\南枫知识库` 尚未执行 v3。正式升级、正式数据上的完整恢复、可见桌面长时间交互和优雅退出仍需独立授权与验证，不得由当前隔离证据替代。
...
| Markdown 展示 | 保真保存源文本，同时以安全 GFM/Obsidian 子集和类型化 callout 展示 | `components/MarkdownContent.tsx` | `MarkdownContent` / `AssistantMessageContent` | 详情、完整内容、编辑预览、历史快照、分享 | 页面各写一套正则 HTML；把全部 callout 套为同一颜色；父级 flex 误作用于 Markdown 根节点；把英文话题当内部过程删除 | 语义类型/别名契约 + 生产构建 + 内容 E2E + 真实长会话单列排版 + 源文不改写 | 已实现 |
| 外部链接 | 软件正文中的外部地址由系统默认浏览器或默认应用打开 | `components/MarkdownContent.tsx` / `tauri-plugin-opener` | `openUrl` | 全部 Markdown 阅读界面 | 让外站替换主 WebView；在应用内嵌不可退出的网页 | 权限/生产构建 + 浏览器新窗口回退 | 已实现 |
