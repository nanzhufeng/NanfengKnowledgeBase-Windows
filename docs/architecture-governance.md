...
| 文件导入 | 原文件归档、哈希、解析预览、映射、去重和写入 | `src-tauri` 导入服务 | `RecordRepository` 导入方法 | 导入中心、导入日志 | UI 直接读取后无归档写库 | 原文件存在 + 哈希/日志/记录一致 | 实施中 |
...
| JSON/Markdown/TXT/HTML 文件导入 | UI 演示、后端待接 | 保真通道 | 待实现导入服务 | 原文件/哈希/日志/记录闭环 |
...
| 导出、备份、恢复 | 待实现 | 受影响 | 待实现 Rust 服务 | 文件结果、完整性与回滚 |
| 不同窗口与 DPI | 浏览器已验、桌面待验 | 受影响 | CSS 响应式规则 / Tauri 窗口 | 浏览器尺寸 + Windows 桌面壳 |
