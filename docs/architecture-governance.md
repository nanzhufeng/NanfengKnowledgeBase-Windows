...
| 记录视图交互状态 | 搜索/标签预设、当前范围、来源筛选、排序、收藏与选中记录的联动 | `App` / `RecordRepository.listRecords` | 组件 props 与仓库查询 | 全部记录、我的收藏、持续跟踪、判断更新、标签菜单、详情标题 | 侧栏和列表各存一份业务状态 | 查询结果、计数、详情与收藏同步 | 已实现 |
