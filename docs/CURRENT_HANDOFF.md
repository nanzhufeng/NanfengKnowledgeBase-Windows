...
> 当前工作区：分类 v3、阅读详情弹窗、空内容默认过滤、性能优化、正式数据复核与本地 NSIS 安装生命周期均已完成对应层级验证
...
完整迁移备份、失败自动回滚、隐藏 Tauri 命令桥和重启持久化已经在隔离根验证。南烛枫于 2026-07-27 分别授权后，正式 `D:\南枫知识库` 已完成 migration v3 和独立只读复核；BAT 已完成知识工作区、长正文、收藏恢复、附件打开入口、隔离批量导入和优雅退出验收；南烛枫已确认性能优化后“流畅度好多了”。本轮针对“内容清晰但大量没有主题”的结构性问题完成分类 v3：个人目录从 20 个扩展为 10 个领域、47 个可编辑主题；标题与用户可见正文共同提供直接证据；BM25 只能增强可见文本中真实出现的词；长正文使用确定性分段采样，仍保持 45/70/90 安全门槛。当前最重要的边界：**v3 BAT 已生成，等待南烛枫关闭仍运行的 v2 测试窗口后验收；正式 901 条来源没有被批量重算，GitHub 发布未执行。**
...
- 收录箱列表默认只加载 120 条轻量摘要，完整正文仅在选中后读取；无正文空会话默认不显示但数据库记录仍保留。
- 超过 20,000 字默认使用流畅预览；正文框内不再叠加遮挡提示，“查看详情”使用与来源档案一致的 85% 大弹窗展示完整正文。
...
- 分类入口通过一个正式 Tauri 命令读取 SQLite 中的主题、别名/实体规则、用户规则、历史确认和归一化 FTS5/BM25 信号；AI 会话先投影为当前分支的用户可见正文，原始 JSON 中的模型名、搜索元数据、附件指针和隐藏内容不进入评分。
- `local-rules-v3` 会同时检查标题与用户可见正文；长正文取首段、中段和末段确定性样本，避免正文越长相似度反而被稀释。候选仍须达到 45 分且具备直接主题证据；历史活跃度和相对全文排名只能增强证据，不能单独制造候选。
- BM25 词只有同时出现在可见标题或正文中才能贡献分数，原始 JSON 隐藏元数据不能借全文索引影响分类。旧算法建议在界面中隐藏，需由用户点击“按新版重新计算”后替换。
- 个人目录 v3 包含 10 个领域、47 个可编辑主题，覆盖投资与楼市、AI 与软件、影视设计、账号支付通信、汽车、健康、教育、生活、文化等主要内容簇。托管规则只使用可判别短语，禁止用 `ChatGPT`、`Claude`、`Google`、`Apple`、`SRT`、`ACES`、`GPU`、`价格`、`视频`、`模型`、`设计`、`账号`、`GitHub`、`银行`、`汽车`、`投资` 等宽泛单词直接归类。
- 首次加载 v3 时只增量添加缺失的托管领域、主题和规则；不会改写来源正文、用户规则、既有确认归类，也不会重复覆盖用户后来停用的托管规则。
...
- 当前 BAT：`启动南枫知识库-测试版.bat`；分类 v3 测试 EXE 位于被忽略的 `.runtime-qa/knowledge-base-build-v3/release/`。
- 可见只读检查确认正式库首屏 120 条、长正文预览和展开/恢复按钮工作；南烛枫已确认流畅度明显改善。
- 分类 v3 BAT 测试 EXE 已在 `.runtime-qa/knowledge-base-build-v3/release/nanfeng-knowledge-base.exe` 生成，大小 15,311,360 字节，SHA-256 为 `8CCAABC9D74FBD08086B40868D08240234A549285D18796190B10459E74A573F`。生成时 v2 测试版仍以 PID 39632 运行，因此采用并列构建槽位，没有强制结束或覆盖运行中程序。
- 对正式库 online backup 的只读标题审计下限：v2 的 20 个主题直接覆盖 83/901（9.2%），v3 的 47 个主题直接覆盖 159/901（17.6%）；正文证据尚未计入这个下限。4 条标题同时命中属于存储芯片与投资风险、台积电与投资风险、海外账号与通信、英伟达护城河与投资风险等合理复合内容。
...
当前知识生产核心代码、隐藏 IPC 工具和正式迁移维护入口对应 checkpoint `6d0180f`；性能优化对应 `d3fae13`、`ab91841`；安装升级修复对应 `8f6559b`；阅读布局、统一正文和前台唤起对应 `9965ad5`；详情弹窗和空内容过滤对应 `eccfd52`；分类 v3 整体覆盖优化对应 `2368887`：

| 验证层级 | 结果 |
|---|---|
| 前端单元/领域合同 | 69/69 通过；包含 11 组用户反馈分类回归、标题直接证据与无证据空候选 |
| Rust/SQLite 合同 | 70/70 通过；包含可见正文投影、空会话过滤、目录 v3 迁移和四条最新截图样本 |
| Sites 回退合同 | 4/4 通过 |
| Playwright 无界面交互 | 18/18 通过；收录箱独立滚动、右侧固定、统一会话卡和“查看详情”大弹窗合同 |
| TypeScript | 通过 |
| Vite 生产构建 | 通过 |
| `git diff --check` | 通过 |
...
| 可见 BAT 用户路径 | 旧版启动、知识工作区、长正文、收藏恢复、附件入口、隔离批量导入、自动目录/建议、独立滚动和优雅退出通过；分类 v3 新 EXE 已生成，待用户验收 |
| 当前版本安装包/升级覆盖/卸载 | 本地 NSIS 全新安装、0.1.0 升级、运行中保护、正式数据保留和卸载通过 |
| 性能优化 BAT 主观体验 | 南烛枫已确认明显改善 |
| 分类 v3 真实内容体验 | 代码、11 组固定样本、Rust/SQLite、构建和无界面交互通过；只读标题覆盖下限由 83 提升到 159，未批量重算正式 901 条来源，待南烛枫用 BAT 抽查 |
...
1. 先关闭仍以 PID 39632 运行的 v2 测试窗口，再双击仓库根 `启动南枫知识库-测试版.bat`。新版 BAT 已指向独立 v3 构建，不需要 `--rebuild`。
2. 首次加载收录箱会幂等增量应用目录 v3，只添加缺失托管主题和规则，不批量重算来源、不覆盖用户规则或既有确认归类。
3. 抽查截图来源：`日本楼市跌幅与分化 → 房地产与楼市研究`、`南枫批量改名设计 → 品牌、图标与界面设计`、`GitHub 分支保护建议 → Git 与版本控制`、`账本恢复失败原因 → 数据备份、迁移与恢复`。点击“按新版重新计算”后应出现证据充分候选。
4. 继续抽查 A股、理想 i8、A 型血、艺术报考、giffgaff、美国银行卡，确认 v2 已修复样本没有回退；无直接证据的内容允许保持未分类，不能为了覆盖率强行误归类。
5. 确认空会话不再出现在默认列表、正文内不再有遮挡条、“查看详情”打开大弹窗，并复核图片/外部链接是否直接在前台。
6. 本地 NSIS 全新安装、旧版升级、运行中保护、正式数据保留和卸载已通过。
7. 南烛枫已明确暂不安装 Inno Setup 7；不要自行切换打包链路。
8. 只有用户明确说“上传”后，才同步默认分支、tag、GitHub Release 和安装器哈希。
...
- 当前代码 checkpoint 为 `2368887`；接手时先以 `git status` 和 `git log -1` 为准，不需要把完整聊天历史重新读取。

## 九、下一件事

当前没有继续扩大代码范围的默认任务。下一道门槛是：**由南烛枫关闭 v2 测试窗口后运行 `启动南枫知识库-测试版.bat`，抽查分类 v3 的四条新样本与既有样本，并继续确认空内容过滤、详情弹窗和图片/外部链接前台显示。**
