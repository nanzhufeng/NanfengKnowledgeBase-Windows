...
> 当前工作区：正式 migration v3 与可见 BAT 验收均已完成对应层级复核
...
完整迁移备份、失败自动回滚、隐藏 Tauri 命令桥和重启持久化已经在隔离根验证。南烛枫于 2026-07-27 单独授权后，正式 `D:\南枫知识库` 已完成 migration v3、完整迁移备份、第二次打开幂等检查和独立只读复核；随后又单独授权打开可见窗口，完成 BAT、知识工作区、长正文、收藏恢复、附件打开入口、隔离批量导入和优雅退出验收。当前最重要的边界：**安装包、升级覆盖、卸载和发布仍未执行。**
...
当前知识生产核心代码、隐藏 IPC 工具和正式迁移维护入口对应 checkpoint `6d0180f`；BAT 启动检测修复对应 `b962159`：
...
1. BAT 的启动、收藏恢复、长正文、PNG/WAV 附件打开入口、知识工作区、隔离批量导入和优雅退出已通过。
2. 用户单独授权后，再重新构建安装包并执行安装、升级覆盖和卸载验证。
...
当前没有继续扩大代码范围的默认任务。下一道门槛是：**等待南烛枫单独授权后，重新构建当前版本安装包，并执行安装、旧版升级覆盖、正式数据保留和卸载验证。**

安装与卸载可能改变系统状态，下一轮必须先说明影响范围并取得明确授权。GitHub 发布仍是再下一道独立门槛，不得随安装包验收自动开始。
