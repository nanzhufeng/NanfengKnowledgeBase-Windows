...
当前知识生产和生产分类信号代码对应 checkpoint `482397e`：

| 验证层级 | 结果 |
|---|---|
| 前端单元/领域合同 | 48/48 通过 |
| Rust/SQLite 合同 | 56/56 通过 |
| Sites 回退合同 | 4/4 通过 |
...
1. 生成可审阅的个人主题目录提案，并由用户在软件中确认后才写入；现有 `prototype-seed-v1` 只可作为覆盖测试，不能成为正式主题树。
2. 增加分类规则、主题别名和实体词典的正式 CRUD；当前分类入口已经消费这些 SQLite 数据，但尚无用户管理界面。
3. 分类纠正需要沉淀为明确反馈记录或新规则；当前操作日志只记录归类结果。
4. 增加主题别名与旧路径重定向；当前合并能保全对象并撤销，但没有完成重定向。
5. 独立 Note CRUD、Proposition 和 Turning Point 尚未形成完整生产用例。
6. 拆分提交不属于当前首版，除非用户重新确认产品边界。
...
- 当前代码 HEAD 为 `482397e`；接手时先以 `git status` 和 `git log -1` 为准，不需要把完整聊天历史重新读取。

## 九、下一件事

下一项唯一开发任务：**个人主题目录提案与分类规则/别名/实体管理闭环。**

继续只使用隔离或内存数据库；不触碰正式数据。正式 migration v3 仍需南烛枫专项授权，安装包和 GitHub 发布仍不在当前范围内。
