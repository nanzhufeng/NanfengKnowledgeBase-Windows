...
> 当前工作区：分类 v2、阅读详情弹窗、空内容默认过滤、性能优化、正式数据复核与本地 NSIS 安装生命周期均已完成对应层级验证
...
完整迁移备份、失败自动回滚、隐藏 Tauri 命令桥和重启持久化已经在隔离根验证。南烛枫于 2026-07-27 分别授权后，正式 `D:\南枫知识库` 已完成 migration v3 和独立只读复核；BAT 已完成知识工作区、长正文、收藏恢复、附件打开入口、隔离批量导入和优雅退出验收；南烛枫已确认性能优化后“流畅度好多了”。本轮根据正式 BAT 截图完成分类 v2：分类只读取用户可见正文，个人目录覆盖常见内容主体，旧版宽泛托管规则会在用户明确重新计算或自动整理时停用，无直接证据不展示候选。当前最重要的边界：**新版 BAT 已生成，等待南烛枫关闭仍运行的旧测试窗口后验收分类、详情弹窗和外部前台唤起；GitHub 发布未执行。**
...
- `local-rules-v2` 要求候选达到 45 分且具备直接主题证据；历史活跃度和相对全文排名只能增强证据，不能单独制造候选。旧版建议在界面中隐藏，需由用户点击“按新版重新计算”后替换。
- 个人目录 v2 新增或细化 `A股与基金市场`、`海外账号体系`、`海外银行与支付`、`海外通信与号码`、`新能源车与二手车`、`血型与输血`、`艺术与设计升学`、`AI 图像与视觉创作`；应用时只停用旧 `catalog-rule-*` 宽泛规则，不覆盖用户规则、正文或既有归类。
...
- 当前 BAT：`启动南枫知识库-测试版.bat`；分类 v2 测试 EXE 位于被忽略的 `.runtime-qa/knowledge-base-build-v2/release/`。
- 可见只读检查确认正式库首屏 120 条、长正文预览和展开/恢复按钮工作；南烛枫已确认流畅度明显改善。
- 分类 v2 BAT 测试 EXE 已在 `.runtime-qa/knowledge-base-build-v2/release/nanfeng-knowledge-base.exe` 生成，大小 15,286,784 字节，SHA-256 为 `D1FBE93A947CB131E17D2CB6A7A7F1EAD532B126D98F9D0DAE44D20541C91B7B`。生成时旧测试版仍由南烛枫保持运行，因此采用并列构建槽位，没有强制结束或覆盖运行中程序。
...
当前知识生产核心代码、隐藏 IPC 工具和正式迁移维护入口对应 checkpoint `6d0180f`；性能优化对应 `d3fae13`、`ab91841`；安装升级修复对应 `8f6559b`；阅读布局、统一正文和前台唤起对应 `9965ad5`；分类 v2、详情弹窗和空内容过滤对应 `eccfd52`：

| 验证层级 | 结果 |
|---|---|
| 前端单元/领域合同 | 64/64 通过；包含 7 组用户反馈分类回归与无证据空候选 |
| Rust/SQLite 合同 | 69/69 通过；包含可见正文投影、空会话过滤和目录 v2 迁移 |
| Sites 回退合同 | 4/4 通过 |
...
| 可见 BAT 用户路径 | 旧版启动、知识工作区、长正文、收藏恢复、附件入口、隔离批量导入、自动目录/建议、独立滚动和优雅退出通过；分类 v2 新 EXE 已生成，待用户验收 |
| 当前版本安装包/升级覆盖/卸载 | 本地 NSIS 全新安装、0.1.0 升级、运行中保护、正式数据保留和卸载通过 |
| 性能优化 BAT 主观体验 | 南烛枫已确认明显改善 |
| 分类 v2 真实内容体验 | 代码、固定截图样本合同、Rust/SQLite、构建和无界面交互通过；未批量重算正式 901 条来源，待南烛枫用 BAT 抽查并决定是否整理已加载来源 |
...
1. 先关闭 17:42 启动且仍运行的旧测试窗口，再双击仓库根 `启动南枫知识库-测试版.bat`。新版 BAT 已指向独立 v2 构建，不需要 `--rebuild`。
2. 抽查截图中的 A股、理想 i8、A 型血、艺术报考、giffgaff、美国银行卡来源：旧建议应隐藏；点击“按新版重新计算”后只保留证据充分候选。该动作会增量应用目录 v2、停用旧托管规则并替换该来源的待确认建议，不改写正文或既有确认归类。
3. 确认空会话不再出现在默认列表、正文内不再有遮挡条、“查看详情”打开大弹窗，并复核图片/外部链接是否直接在前台。
4. 本地 NSIS 全新安装、旧版升级、运行中保护、正式数据保留和卸载已通过。
5. 南烛枫已明确暂不安装 Inno Setup 7；不要自行切换打包链路。
6. 只有用户明确说“上传”后，才同步默认分支、tag、GitHub Release 和安装器哈希。
...
- 当前代码 checkpoint 为 `eccfd52`；接手时先以 `git status` 和 `git log -1` 为准，不需要把完整聊天历史重新读取。

## 九、下一件事

当前没有继续扩大代码范围的默认任务。下一道门槛是：**由南烛枫关闭旧测试窗口后运行 `启动南枫知识库-测试版.bat`，抽查分类 v2、空内容过滤与详情弹窗，并确认图片和外部链接是否直接出现在前台。**
