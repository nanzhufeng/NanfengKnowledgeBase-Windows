...
- Windows 正式数据默认写入 `D:\南枫情报台`；目标为空时使用 SQLite 在线备份一致性迁移旧数据库并复制其他受控目录，旧目录保留；D 盘不可用或迁移失败时回退旧目录。
...
- 外部 `http/https/mailto/tel` 链接统一交给系统默认应用打开，不再替换南枫情报台主 WebView；软件内所有可滚动区域使用透明轨道的轻量滚动条。
