...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\skin-layout-catalog-v46-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\skin-layout-catalog-v46-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Skin Layout QA"
    ...
    if ($Prepare) {
        ...
        Write-Host "[PASS] Isolated skin layout QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Skin layout launcher and isolated QA application are available." -ForegroundColor Green
        Write-Host "The application and formal data were not opened." -ForegroundColor Green
        exit 0
    }
