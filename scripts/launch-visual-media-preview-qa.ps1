...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\visual-media-preview-v86-app"
...
if ($Prepare) {
  ...
  if ($LASTEXITCODE -ne 0) { throw "Isolated visual-media preview QA build failed: $LASTEXITCODE" }
  ...
  Write-Host "[PASS] Visual-media preview QA application generated." -ForegroundColor Green
