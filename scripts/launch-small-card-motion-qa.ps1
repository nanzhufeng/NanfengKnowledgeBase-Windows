...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\individual-small-card-motion-v77-app"
...
$logDirectory = Join-Path $projectDirectory ".runtime-qa\individual-small-card-motion-v77-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Individual Small Card Motion and Shadow QA"
    New-Item -ItemType Directory -Path $logDirectory -Force | Out-Null
    $startedAt = Get-Date -Format "yyyy-MM-dd HH:mm:ss.fff"
    Set-Content -LiteralPath $launchLog -Value "[$startedAt] launcher started" -Encoding UTF8
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building the isolated v77 individual small-card motion and shadow QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated v77 individual small-card motion and shadow QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Individual small-card motion and shadow launcher and isolated QA application are available." -ForegroundColor Green
