...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\source-heading-removal-v44-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\source-heading-removal-v44-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Source Heading Removal QA"
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building isolated source-heading-removal QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated source-heading-removal QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Source-heading-removal launcher and isolated QA application are available." -ForegroundColor Green
