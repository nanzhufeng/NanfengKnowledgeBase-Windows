...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\source-delete-persistence-v47-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\source-delete-persistence-v47-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Source Delete Persistence QA"
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building isolated source-delete-persistence QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated source-delete-persistence QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Source-delete-persistence launcher and isolated QA application are available." -ForegroundColor Green
