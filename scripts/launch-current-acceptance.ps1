...
$buildDirectory = Join-Path $runtimeDirectory "current-acceptance-v93-build"
$targetDirectory = Join-Path $runtimeDirectory "current-acceptance-v93-app"
...
if ($Prepare) {
  ...
  try {
    ...
    $env:NF_BUILD_LABEL = "v93-current-acceptance"
    ...
    Write-Host "[PASS] v93 current acceptance app generated. App and formal data were not opened." -ForegroundColor Green
  } finally {
    $buildLock.Dispose()
  }
  exit 0
...
if ($Verify) {
  ...
  Write-Host "[PASS] v93 current acceptance app is available." -ForegroundColor Green
  Write-Host "Application: $applicationPath"
  Write-Host "Build label: v93-current-acceptance"
