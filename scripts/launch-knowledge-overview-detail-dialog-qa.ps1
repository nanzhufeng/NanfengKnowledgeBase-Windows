...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\knowledge-overview-dialog-v42-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\knowledge-overview-dialog-v42-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Overview Detail Dialog QA"
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building isolated overview-detail-dialog QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated overview-detail-dialog QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Overview-detail-dialog launcher and isolated QA application are available." -ForegroundColor Green
