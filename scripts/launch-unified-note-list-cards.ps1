...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\unified-note-list-v18-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\unified-note-list-v18-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Unified Note List QA"
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building isolated unified-note-list QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated unified-note-list QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Unified-note-list launcher and isolated QA application are available." -ForegroundColor Green
        Write-Host "The application and formal data were not opened." -ForegroundColor Green
        exit 0
    }
    ...
    # Visible user QA defaults to option 1. Codex only runs Prepare or Verify.
