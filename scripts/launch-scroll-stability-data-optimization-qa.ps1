...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\scroll-stability-data-optimization-v75-app"
...
$logDirectory = Join-Path $projectDirectory ".runtime-qa\scroll-stability-data-optimization-v75-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Scroll Stability and Data Optimization QA"
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building isolated v75 QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated v75 QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] v75 launcher and isolated QA application are available." -ForegroundColor Green
    ...
    # 只有用户可见启动才使用默认正式数据目录；Codex 只执行 Prepare / Verify。
