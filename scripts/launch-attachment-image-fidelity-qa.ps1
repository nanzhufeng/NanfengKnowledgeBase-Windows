...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\attachment-image-fidelity-v48-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\attachment-image-fidelity-v48-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Original Image Fidelity QA"
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building isolated original-image-fidelity QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated original-image-fidelity QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Original-image-fidelity launcher and isolated QA application are available." -ForegroundColor Green
