...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\three-entry-linkage-v3-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\three-entry-linkage-v3-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Three Entry Linkage QA"
    ...
    # The visible launcher defaults to the formal data location, as requested.
    # Codex only runs --prepare/--verify and never opens the formal data.
