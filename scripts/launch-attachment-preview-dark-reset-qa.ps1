...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\attachment-preview-dark-reset-v43-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\attachment-preview-dark-reset-v43-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Dark Image Preview QA"
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building isolated dark-image-preview QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated dark-image-preview QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Dark-image-preview launcher and isolated QA application are available." -ForegroundColor Green
