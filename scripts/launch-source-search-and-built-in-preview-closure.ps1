...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\source-search-preview-v17-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\source-search-preview-v17-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Search and Preview QA"
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building isolated source-search and built-in-preview QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated source-search and built-in-preview QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Search-and-preview launcher and isolated QA application are available." -ForegroundColor Green
        Write-Host "The application and formal data were not opened." -ForegroundColor Green
        exit 0
    }
    ...
    # User-visible QA defaults to option 1: the normal data location. Codex only runs Prepare or Verify.
