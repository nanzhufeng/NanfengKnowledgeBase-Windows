...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\card-two-viewport-v85-app"
...
if ($Prepare) {
  ...
  if ($LASTEXITCODE -ne 0) { throw "Isolated card-two viewport QA build failed: $LASTEXITCODE" }
  ...
  Write-Host "[PASS] Card-two viewport QA application generated." -ForegroundColor Green
