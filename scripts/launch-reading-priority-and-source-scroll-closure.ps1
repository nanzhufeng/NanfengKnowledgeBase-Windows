...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\reading-priority-v16-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\reading-priority-v16-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Reading Priority QA"
    ...
    if ($Prepare) {
        ...
        Write-Host "[PASS] Isolated reading-priority QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Reading-priority launcher and isolated QA application are available." -ForegroundColor Green
        Write-Host "The application and formal data were not opened." -ForegroundColor Green
        exit 0
    }
