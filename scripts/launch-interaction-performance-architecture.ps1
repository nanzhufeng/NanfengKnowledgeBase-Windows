...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\interaction-performance-v45-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\interaction-performance-v45-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Interaction Performance QA"
    ...
    if ($Prepare) {
        ...
        Write-Host "[PASS] Isolated performance QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Performance launcher and isolated QA application are available." -ForegroundColor Green
        Write-Host "The application and formal data were not opened." -ForegroundColor Green
        exit 0
    }
