...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\knowledge-overview-v40-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\knowledge-overview-v40-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Overview QA"
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building isolated knowledge-overview QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated knowledge-overview QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Knowledge-overview launcher and isolated QA application are available." -ForegroundColor Green
