...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\feedback-13-items-v4-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\feedback-13-items-v4-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - 13 Feedback Items QA"
    ...
    # Visible QA defaults to the formal data location without a 1/2 prompt.
    # Codex only runs -Prepare or -Verify and never launches formal data.
