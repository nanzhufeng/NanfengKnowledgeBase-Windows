...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\knowledge-interaction-closure-v2-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\knowledge-interaction-closure-v2-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Interaction Closure QA"
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Launcher and QA application are available." -ForegroundColor Green
    ...
    if ($Rebuild -or -not (Test-Path -LiteralPath $applicationPath -PathType Leaf)) {
        ...
        Write-LaunchLog "Building isolated QA executable."
    ...
    # The user requested no 1/2 prompt. Default mode is the formal data location.
    # This launcher is only generated and verified by Codex; the user starts it visibly.
    ...
    try {
        ...
        # Logging errors must not hide the original launcher failure.
    }
