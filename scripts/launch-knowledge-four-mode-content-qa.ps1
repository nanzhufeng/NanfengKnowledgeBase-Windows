...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\knowledge-four-mode-content-v41-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\knowledge-four-mode-content-v41-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "Nanfeng Knowledge - Four Mode Content QA"
    ...
    if ($Prepare) {
        ...
        Write-LaunchLog "Building isolated four-mode-content QA executable without opening the application."
        ...
        Write-Host "[PASS] Isolated four-mode-content QA application generated." -ForegroundColor Green
    ...
    if ($Verify) {
        ...
        Write-Host "[PASS] Four-mode-content launcher and isolated QA application are available." -ForegroundColor Green
