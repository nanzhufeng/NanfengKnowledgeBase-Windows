...
$targetDirectory = Join-Path $projectDirectory ".runtime-qa\knowledge-ownership-closure-v1-build"
$applicationPath = Join-Path $targetDirectory "release\nanfeng-knowledge-base.exe"
$logDirectory = Join-Path $projectDirectory ".runtime-qa\knowledge-ownership-closure-v1-launcher"
...
try {
    $Host.UI.RawUI.WindowTitle = "南枫知识库 - 产品职责收口验收"
    New-Item -ItemType Directory -Path $logDirectory -Force | Out-Null
    Set-Content -LiteralPath $launchLog -Value "[$(Get-Date -Format "yyyy-MM-dd HH:mm:ss.fff")] 启动产品职责收口验收版" -Encoding UTF8
    Write-LaunchLog "项目目录：$projectDirectory"
    Write-LaunchLog "程序路径：$applicationPath"

    if ($Verify) {
        if (-not (Test-Path -LiteralPath $applicationPath -PathType Leaf)) {
            throw "未找到产品职责收口验收程序：$applicationPath"
        }
        Write-LaunchLog "仅验证启动器与程序路径；未启动程序"
        Write-Host "[通过] 启动器和产品职责收口验收程序路径有效。" -ForegroundColor Green
        Write-Host "未启动程序，也未打开任何正式数据。" -ForegroundColor Green
        exit 0
    }
    ...
    if ($Rebuild -or -not (Test-Path -LiteralPath $applicationPath -PathType Leaf)) {
        if (-not (Get-Command npm.cmd -ErrorAction SilentlyContinue)) {
            throw "未找到 npm，无法生成产品职责收口验收程序。"
        }
        Write-Host "[准备] 正在生成产品职责收口验收程序，不会创建或安装安装包……" -ForegroundColor Cyan
    ...
    # 南烛枫已明确：可见验收默认使用正式数据，不再弹出数据模式选择。
    ...
    Write-Host "[启动] 南枫知识库产品职责收口验收版（正式数据）" -ForegroundColor Cyan
    Write-LaunchLog "数据模式：正式数据"
